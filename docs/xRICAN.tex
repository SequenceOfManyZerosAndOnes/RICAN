...
\title{RICAN: A Fixed-Rate Representation Method for Finite Alphabets via Continued-Fraction Approximation}
...
\begin{abstract}
This paper presents a general methodology for constructing asymptotically efficient fixed-length binary representations for sequences over finite alphabets. The core contribution is a systematic method, based on Diophantine approximation via continued fractions, for selecting block sizes that achieve near-minimal representation overhead for any alphabet size $B$.
...
For the decimal case ($B=10$), the convergent $2136/643$ yields $K=643$ decimal digits per block, represented in exactly $267$ bytes ($2136$ bits), with a per-symbol ceiling redundancy of $3.65\times10^{-7}$ bits, obtained from the block overhead $\varepsilon \approx 2.35\times10^{-4}$ bits. This denominator achieves an approximation error below $10^{-6}$.

Unlike statistical compression methods, this approach does not exploit source redundancy. It provides a deterministic, reversible mapping whose output size depends only on the input length and alphabet size, not on statistical properties. Validation on up to $10^9$ decimal digits confirms the theoretical efficiency and linear computational complexity for fixed block size.
\end{abstract}

\begin{IEEEkeywords}
Fixed-rate source coding, radix conversion, finite alphabets, continued fractions, Diophantine approximation, block encoding.
\end{IEEEkeywords}
...
\end{axis}
\end{tikzpicture}
\caption{Approximation error of the convergents of $\log_2(10)$ in logarithmic scale. The convergent $2136/643$ produces a dramatic reduction in error, dropping from approximately $10^{-4}$ to $10^{-7}$. The dashed line indicates the $10^{-6}$ threshold; $643$ is the first denominator that falls below this threshold.}
\label{fig:error}
\end{figure}
...
\item A practical instantiation for decimal digits ($B=10$) with $K=643$ and its experimental validation.
...
\subsection{Application to Decimal Digits}

For $B=10$, the continued fraction expansion yields the convergent

\[
\frac{2136}{643}
\]

with error $3.65\times10^{-7}$. The value $3.65\times10^{-7}$ is obtained from

\[
\frac{2136}{643} - \log_2(10) \approx 3.65\times10^{-7}.
\]

Among the first convergents, $643$ is a denominator with error below $10^{-6}$. Consequently,

\[
643 \cdot \log_2(10) = 2136 - \varepsilon,
\]

where $\varepsilon \approx 2.35 \times 10^{-4}$ is the block overhead. The per-symbol ceiling redundancy is

\[
\frac{\varepsilon}{643} \approx 3.65\times10^{-7}.
\]

The denominator determines the block size, whereas the numerator simply determines the corresponding bit length. For implementation, convergents approaching from above are preferable because they directly determine the ceiling representation length.

\begin{table}[ht]
\centering
\caption{Convergents of $\log_2(10)$}
\label{tab:convergents}
\begin{tabular}{cccc}
\toprule
Index & Convergent & Value & Error \\
\midrule
0 & $1/1$ & 1.000000000 & $2.3219$ \\
1 & $3/1$ & 3.000000000 & $0.3219$ \\
2 & $10/3$ & 3.333333333 & $0.0114$ \\
3 & $93/28$ & 3.321428571 & $4.995\times10^{-4}$ \\
4 & $196/59$ & 3.322033898 & $1.058\times10^{-4}$ \\
5 & $2136/643$ & 3.321928460 & $3.654\times10^{-7}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item Example datasets (1M, 10M, and 100M digits of $\pi$)
    \item All figures from the paper
    \item Benchmark results
    \item LaTeX source of this manuscript
\end{itemize}

The specific version used for this publication is archived at:
\begin{center}
\url{https://github.com/SequenceOfManyZerosAndOnes/RICAN}
\end{center}
